...
### CSS Maintainability
New in version 1.0.1 - CSS maintainability is managed by layering all the SCSS in logical groups. We tried to follow standards like SMACSS or ITCSS, but they didn't quite match our needs. We thought this project was small enough, and we wanted to feel confortable, so we made up our own file structure: resets > variables > layout > tools > elements > theme. Most of these groups could match one or other category in the methods mentioned above, except for 'variables'. The aim of this file is to gather all the theming parameters used across the other files, so one can quickly tweak the overall design in one place. If we had to find a name for this new method, I would probably call it Parametric ITCSS (or PITCSS).
